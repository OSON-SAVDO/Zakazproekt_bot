...
def run():
    # Гирифтани порт аз система (барои Render)
...
def start(message):
    bot.send_message(
        message.chat.id, 
        f"Салом {message.from_user.first_name}! Ба боти сартарошхона хуш омадед.", 
        reply_markup=main_menu()
    )

# --- НАРХНОМА ВА ХИЗМАТРАСОНИҲО ---
...
def show_time_slots(message):
    if not message.contact:
        bot.send_message(message.chat.id, "❌ Хатогӣ: Рақам фиристода нашуд. Лутфан тугмаро пахш кунед.", reply_markup=main_menu())
        return
    ...
    if busy_slots:
        ...
        busy_text = "✅ Ҳама вақтҳо ҳоло холианд."
        
    msg = bot.send_message(
        message.chat.id, 
        f"{busy_text}\n\nКадом вақт меоед? (масалан: 14:30):", 
...
def final_booking(message, phone):
    ...
    if user_time in bookings:
        msg = bot.send_message(message.chat.id, "❌ Ин вақт банд аст. Вақти дигар нависед (масалан 15:00):")
        bot.register_next_step_handler(msg, final_booking, phone)
    else:
...
def cancel_booking(message):
    ...
    if not user_slots:
        ...
        for s in user_slots:
            m.add(types.InlineKeyboardButton(f"🗑 Бекор кардани {s}", callback_data=f"u_del_{s}"))
...
def admin_panel(message):
    if message.chat.id == MY_ID:
        if not bookings:
            bot.send_message(MY_ID, "Ҳоло ягон навбат нест.")
        else:
            m = types.InlineKeyboardMarkup()
            for s in bookings.keys():
                m.add(types.InlineKeyboardButton(f"❌ Холӣ кардани {s}", callback_data=f"a_del_{s}"))
            bot.send_message(MY_ID, "Рӯйхати навбатҳо:", reply_markup=m)

@bot.callback_query_handler(func=lambda call: call.data.startswith('a_del_'))
def a_del_callback(call):
    if call.message.chat.id == MY_ID:
        s = call.data.split('_')[2]
        if s in bookings:
            del bookings[s]
